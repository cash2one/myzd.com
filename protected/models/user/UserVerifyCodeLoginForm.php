...
class UserVerifyCodeLoginForm extends EFormModel {

    public $username;
    public $duration = 7776000; // 90天
    public $verify_code;
    public $role;
    public $errorFormCode = UserVerifycodeIdentity::ERROR_NONE;
    public $_identity;
    ...
    public function authenticate() {
        if ($this->hasErrors() === false) {   // no validation error.            
            ...
            $this->_identity = new UserVerifycodeIdentity($this->username, $this->role);
            if ($this->_identity->authenticate() === false) {
                $this->errorFormCode = $this->_identity->errorCode;
                if ($this->errorFormCode == UserVerifycodeIdentity::ERROR_ACCOUNT_NOT_ACTIVATED) {
                    $url = Yii::app()->createAbsoluteUrl('user/resendActivation');
                    $this->addError('username', "您的帐号还没有激活。<br /><a href='$url'>现在去激活</a>");
                } elseif ($this->errorFormCode == UserVerifycodeIdentity::ERROR_USERNAME_INVALID) {
                    if ($this->autoRegister === false) {
                        $this->addError('username', '该用户名不存在');
                    }
                }
            } else {
                $this->errorFormCode = UserVerifycodeIdentity::ERROR_NONE;
            }
        }
    }

    public function verifyCodeForUserLogin() {
        $authMgr = new AuthManager();
        $userHostIp = null;
        $this->authSmsVerify = $authMgr->verifyCodeForUserLogin($this->username, $this->verify_code, $userHostIp);
    }
