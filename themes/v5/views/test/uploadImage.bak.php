 ...
 */
$tripId = 1;
$loadImagesUrl = $this->createUrl('test/ajaxLoadImages', array('id' => $tripId, 'tmpl' => 1));
...
<div class="container">
    ...
    <form id="fileupload" action="<?php echo $uploadUrl; ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="row fileupload-buttonbar">
            <div class="col-lg-7">
                ...
                <span class="btn btn-success fileinput-button">
                    ...
                    <span>Add files...</span>
                    <input type="file" name="image_uploads[]" multiple>
                </span>
...
<script id="template-download" type="text/x-tmpl">
    ...
    <tr class="template-download fade {% if(o.init){ %} in {% } %}">
        ...
        <td>
            <input onclick="setCoverImage({%= file.id %});" type="radio" name="coverImage" value="{%=file.id%}" {% if(file.isCover){ %} checked {% } %}>封面
        </td>
        <td>
            <p class="name">
                {% if (file.url) { %}
        ...
        <td>
            {% if (file.deleteUrl) { %}
            <button class="btn btn-danger delete" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}" data-data="id={%=file.tripId%}&imageId={%=file.id%}" {% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}>
                    <i class="glyphicon glyphicon-trash"></i>
                <span>Delete</span>
            </button>
...
<script>
    $(document).ready(function(){
      ...
      $(".template-download input[type=radio]").click(function(e){
          alert($(this).val());
          return false;
      });
    });
    ...
    function setCoverImage(id){
        ...
        $.ajax({
            type:'post',
            dataType:"json",
            url:"<?php echo $setCoverUrl;?>",
            data:'TripImage[id]='+id,
            success:function(data){
